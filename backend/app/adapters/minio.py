...
class MinIOAdapter(StorageAdapter):
    def __init__(self):
        ...
        self.mock_dir = os.path.join(settings.MOCK_STORAGE_DIR, "minio")
    ...
    def upload(self, file_data: bytes, file_name: str, bucket_name: str) -> bool:
        try:
            ...
            # Fallback to local file simulation
            try:
        ...
        try:
            ...
            if os.path.exists(path):
                with open(path, "rb") as f:
                    return f.read()
            raise FileNotFoundError(f"File {file_name} not found in MinIO bucket {bucket_name}")
